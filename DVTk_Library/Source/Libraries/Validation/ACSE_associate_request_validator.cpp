...
ASSOCIATE_RQ_VALIDATOR_CLASS::ASSOCIATE_RQ_VALIDATOR_CLASS()
...
ACSE_APPL_CTX_NAME_CLASS& ASSOCIATE_RQ_VALIDATOR_CLASS::getApplicationContextName()
...
UINT ASSOCIATE_RQ_VALIDATOR_CLASS::noPresentationContexts()
...
ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS& ASSOCIATE_RQ_VALIDATOR_CLASS::getPresentationContext(UINT i)
...
ACSE_USER_INFORMATION_VALIDATOR_CLASS& ASSOCIATE_RQ_VALIDATOR_CLASS::getUserInformation()
...
ACSE_PARAMETER_CLASS *ASSOCIATE_RQ_VALIDATOR_CLASS::getProtocolVersionParameter()
...
ACSE_PARAMETER_CLASS *ASSOCIATE_RQ_VALIDATOR_CLASS::getCalledAeTitleParameter()
...
ACSE_PARAMETER_CLASS *ASSOCIATE_RQ_VALIDATOR_CLASS::getCallingAeTitleParameter()
...
bool ASSOCIATE_RQ_VALIDATOR_CLASS::validate(ASSOCIATE_RQ_CLASS * srcAssocRq_ptr, ASSOCIATE_RQ_CLASS * refAssocRq_ptr, ACSE_PROPERTIES_CLASS * acseProp_ptr)

//  DESCRIPTION     : Validate Associate Request.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      : 
//  NOTES           :
//<<===========================================================================		
{
	...
	if (srcAssocRq_ptr == NULL) return false;
	
	// check if reference provided
	if (refAssocRq_ptr)
	{
		// set up reference values
		if (refAssocRq_ptr->getProtocolVersion() != UNDEFINED_PROTOCOL_VERSION)
		{
			sprintf(buffer, "%d", refAssocRq_ptr->getProtocolVersion());
			refProtocolVersion = buffer;
		}
		
		if (refAssocRq_ptr->getCalledAeTitle())
		{
			refCalledAeTitle = refAssocRq_ptr->getCalledAeTitle();
		}
		if (refAssocRq_ptr->getCallingAeTitle())
		{
			refCallingAeTitle = refAssocRq_ptr->getCallingAeTitle();
		}
		if (refAssocRq_ptr->getApplicationContextName().get())
		{
			refApplicationContextName = (char*) refAssocRq_ptr->getApplicationContextName().get();
		}
		refUserInformation_ptr = &refAssocRq_ptr->getUserInformation();
	}
	...
	sprintf(buffer, "%d", srcAssocRq_ptr->getProtocolVersion());
	bool result1 = protocolVersionM.validate(buffer, refProtocolVersion);
	bool result2 = calledAeTitleM.validate(srcAssocRq_ptr->getCalledAeTitle(), refCalledAeTitle);
	bool result3 = callingAeTitleM.validate(srcAssocRq_ptr->getCallingAeTitle(), refCallingAeTitle);
	
	bool result4 = applicationContextNameM.validate((char*) srcAssocRq_ptr->getApplicationContextName().get(), refApplicationContextName);

	// pre-process presentation contexts by flaging all as unvalidated
	setUnvalidatedFlags(srcAssocRq_ptr, refAssocRq_ptr, true);
	
	bool result5 = true;
	for (UINT i = 0; i < srcAssocRq_ptr->noPresentationContexts(); i++)
	{
		PRESENTATION_CONTEXT_RQ_CLASS *pcContext_ptr = &srcAssocRq_ptr->getPresentationContext(i);
		ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS pcValidator;
		
		// validate individual presentation contexts
		if (!pcValidator.validate(pcContext_ptr, refAssocRq_ptr))
		{
			result5 = false;
		}
	...
	bool result6 = checkIfValidated(srcAssocRq_ptr, refAssocRq_ptr);
	
	// post-process presentation contexts by flaging all as validated
	setUnvalidatedFlags(srcAssocRq_ptr, refAssocRq_ptr, false);
	...
	bool result9 = userInformationM.validate(&srcAssocRq_ptr->getUserInformation(),
...
void ASSOCIATE_RQ_VALIDATOR_CLASS::setUnvalidatedFlags(ASSOCIATE_RQ_CLASS *srcAssocRq_ptr, ASSOCIATE_RQ_CLASS *refAssocRq_ptr, bool flag)
...
//<<===========================================================================		
{
	...
	if (srcAssocRq_ptr)
	{
		for (UINT i = 0; i < srcAssocRq_ptr->noPresentationContexts(); i++)
		{
			srcAssocRq_ptr->getPresentationContext(i).setReserved1(reserved1);
		}
	}
	...
	if (refAssocRq_ptr)
	{
		...
		{
			refAssocRq_ptr->getPresentationContext(i).setReserved1(reserved1);
		}
	}
}
...
bool ASSOCIATE_RQ_VALIDATOR_CLASS::checkIfValidated(ASSOCIATE_RQ_CLASS *srcAssocRq_ptr, ASSOCIATE_RQ_CLASS *refAssocRq_ptr)
...
//<<===========================================================================		
{
	...
	if ((srcAssocRq_ptr) &&
		(refAssocRq_ptr) &&
		(refAssocRq_ptr->noPresentationContexts()))
	{
		// only need to check source if a reference is defined
		for (UINT i = 0; i < srcAssocRq_ptr->noPresentationContexts(); i++)
		{
			PRESENTATION_CONTEXT_RQ_CLASS pcContext = srcAssocRq_ptr->getPresentationContext(i);
	...
	if (refAssocRq_ptr)
	{
		for (UINT i = 0; i < refAssocRq_ptr->noPresentationContexts(); i++)
		{
			PRESENTATION_CONTEXT_RQ_CLASS pcContext = refAssocRq_ptr->getPresentationContext(i);
...
void ASSOCIATE_RQ_VALIDATOR_CLASS::updatePresentationContext(PRESENTATION_CONTEXT_RQ_CLASS *srcPC_ptr)
...
//<<===========================================================================		
{
	...
	{
		ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS& pcValidator = getPresentationContext(i);
		
		// check for match
		if ((pcValidator.getPresentationContextId() == pcId) &&
			(pcValidator.getAbstractSyntaxName() == (char*) srcPC_ptr->getAbstractSyntaxName().get()) &&
			(pcValidator.noTransferSyntaxNames() == srcPC_ptr->noTransferSyntaxNames()))
		{
			bool matched = true;
			
			// check that the transfer syntax names match
			for (UINT j = 0; j < pcValidator.noTransferSyntaxNames(); j++)
			{
				UID_CLASS uid((char*) pcValidator.getTransferSyntaxName(j).c_str());
				if (!srcPC_ptr->isTransferSyntaxName(uid))
				{
					matched = false;
					break;
				}
...
bool ASSOCIATE_RQ_VALIDATOR_CLASS::checkUniquePresentationContextId()
...
//<<===========================================================================		
{
    ...
    {
		ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS& pcValidator = getPresentationContext(0);
...
bool ASSOCIATE_RQ_VALIDATOR_CLASS::checkUniqueSOPClasses(void)
...
//<<===========================================================================		
{
	...
	for (i=0 ; i<noPC ; i++)
	{
		ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS pcValidator1 = getPresentationContext(i);
		
		for (j=i+1 ; j<noPC ; j++)
		{
			ACSE_PRESENTATION_CONTEXT_RQ_VALIDATOR_CLASS& pcValidator2 = getPresentationContext(j);
			
			// check for match
			if ((pcValidator1.getAbstractSyntaxName() == pcValidator2.getAbstractSyntaxName()) &&
				(pcValidator1.equalTransferSyntaxes(pcValidator2)))
			{
