    ...
    case ATTR_TYPE_1:
			...
			(dcmAttributeM_ptr->getPaddedLength() == 0))
        {
				...
				(defAttributeM_ptr->GetVR() == ATTR_VR_OW)))
			{
				...
				{
					sprintf(message,
						"OB/OF/OW Data length is zero - Type 1 (mandatory) attribute (%04X,%04X) should have at least one value",
						defAttributeM_ptr->GetGroup(),
						defAttributeM_ptr->GetElement());
...
//  DESCRIPTION     : Check if the OB/OF/OW data is present or not.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      :
//  NOTES           :
