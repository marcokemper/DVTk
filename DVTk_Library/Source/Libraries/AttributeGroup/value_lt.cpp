...
DVT_STATUS VALUE_LT_CLASS::Set(string value)
...
DVT_STATUS VALUE_LT_CLASS::Set (unsigned char *value, UINT32 size)

//  DESCRIPTION     : Set value to size given.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      :
//  NOTES           :
...
DVT_STATUS VALUE_LT_CLASS::Get (unsigned char **value, UINT32 &size)
...
DVT_STATUS VALUE_LT_CLASS::Get(string &value, bool)
...
bool VALUE_LT_CLASS::operator = (BASE_VALUE_CLASS &value)
...
//<<===========================================================================
{
    ...
    if (sizeM > LT_LENGTH)
    {
        sprintf (message, "LT - value length %i exceeds maximum length %i",
                 sizeM, LT_LENGTH);
        messages->AddMessage (VAL_RULE_D_LT_1, message);
        result = MSG_ERROR;
    }

    // check the VR of the value
    DVT_STATUS result1 = EXTCHARACTERSET->IsValidExtendedChar(valueM, sizeM, ATTR_VR_LT, LT_CHAR_LENGTH, messages, specific_character_set);
...
DVT_STATUS VALUE_LT_CLASS::Compare(LOG_MESSAGE_CLASS*, BASE_VALUE_CLASS *ref_value)
...
//<<===========================================================================
{
    ...
    if (ref_value->GetVRType() != ATTR_VR_LT)
    {
...
ATTR_VR_ENUM VALUE_LT_CLASS::GetVRType(void)
...
UINT32 VALUE_LT_CLASS::GetLength (void)
