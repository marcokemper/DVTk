...
namespace DCMEditor
{
	...
	public class AddAttribute : System.Windows.Forms.Form
	{
		...
		private void buttonOk_Click(object sender, System.EventArgs e)
		{
			bool ok = true;
            int hexCount = 0;
            string temporaryAttributeTag;
            string hexArray =  "abcdefABCDEF";

            temporaryAttributeTag = textBox_attrTag.Text;

            if (!temporaryAttributeTag.StartsWith("("))
            {
                temporaryAttributeTag = "(" + temporaryAttributeTag;
            }
            if (!temporaryAttributeTag.EndsWith(")"))
            {
                temporaryAttributeTag = temporaryAttributeTag + ")";
            }


            for (int i = temporaryAttributeTag.IndexOf('(') + 1; i < temporaryAttributeTag.LastIndexOf(')'); i++)
            {
                if ((char.IsDigit(temporaryAttributeTag[i])) || hexArray.Contains(char.ToString(temporaryAttributeTag[i])))
                {
                    hexCount += 1;
                    attributeTag += char.ToLower(temporaryAttributeTag[i]); ;
                }
            ...
            if (hexCount == 8)
            {
                ...
                attributeTag = attributeTag.Insert(5, ",");
            }
            else
            {
                MessageBox.Show("Provide valid tag.Attribute could not be added", "Invalid Tag!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                this.Close();
            }
