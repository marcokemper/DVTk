...
OF_VALUE_STREAM_CLASS::OF_VALUE_STREAM_CLASS()
...
bool OF_VALUE_STREAM_CLASS::SetFilename(string filename)
...
//<<===========================================================================
{
	...
	if (strstr(filename.c_str(), "F32B")) 
	{
		// OF, 32 bit data, big endian
		fs_codeM = TS_BIG_ENDIAN;
	}
	else if (strstr(filename.c_str(), "F32L")) 
	{
		// OF, 32 bit data, little endian
		fs_codeM = TS_LITTLE_ENDIAN;
	}
	else {
		// default to OF, 32 bit data, endian unimportant
#ifdef _WINDOWS
		fs_codeM = TS_LITTLE_ENDIAN;
#else
		fs_codeM = TS_BIG_ENDIAN;
...
ATTR_VR_ENUM OF_VALUE_STREAM_CLASS::GetVR()
...
bool OF_VALUE_STREAM_CLASS::IsByteSwapRequired()

//  DESCRIPTION     : Check if byte swap is required in Other Float Data.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      : 
//  NOTES           :
//<<===========================================================================
{
	...
	if (loggerM_ptr)
	{
	    if (byte_swap) 
	    {
            loggerM_ptr->text(LOG_INFO, 1, "Byte Swap of original OF data needed - details:");
        }
        else
        {
            loggerM_ptr->text(LOG_INFO, 1, "No Byte Swap of original OF data needed - details:");
        }
        switch(fs_codeM)
        {
...
void OF_VALUE_STREAM_CLASS::SwapBytes(BYTE* buffer_ptr, UINT32 length)
...
bool OF_VALUE_STREAM_CLASS::StreamPatternTo(DATA_TF_CLASS& data_transfer)
...
DVT_STATUS OF_VALUE_STREAM_CLASS::Compare(LOG_MESSAGE_CLASS *message_ptr, OF_VALUE_STREAM_CLASS &refOfStream)
...
//<<===========================================================================
{
    ...
    if (byteSwap)
    {
        ...
        sprintf (message, "Source OF File data being byte swapped before comparison with Reference OF data");
