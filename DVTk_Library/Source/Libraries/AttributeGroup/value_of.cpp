...
bool VALUE_OF_CLASS::operator = (BASE_VALUE_CLASS &value)
...
UINT32 VALUE_OF_CLASS::GetLength (void)
...
ATTR_VR_ENUM VALUE_OF_CLASS::GetVRType (void)
...
DVT_STATUS VALUE_OF_CLASS::Compare (LOG_MESSAGE_CLASS *message_ptr, BASE_VALUE_CLASS *refValue)
...
//<<===========================================================================
{
    ...
    if (refValue->GetVRType() != ATTR_VR_OF)
    {
        sprintf (message, "Incompatible Reference VR for OF data comparison");
        if (message_ptr) message_ptr->AddMessage(VAL_RULE_D_OTHER_2, message);
        return MSG_INCOMPATIBLE;
    }
    VALUE_OF_CLASS *refOfValue = static_cast<VALUE_OF_CLASS*>(refValue);
        ...
        (refOfValue->filenameM.length()))
    {
        ...
        else
        {
            ...
            if (srcStream.GetFileVR() == ATTR_VR_OF)
            {
                ...
                if (refStream.GetFileVR() == ATTR_VR_OF)
                {
        ...
        (refOfValue->filenameM.length()))
    {
        ...
        // - can't compare
        sprintf (message, "Can't compare In-File and Pattern OF data");
    ...
    else
    {
        ...
        {
            sprintf (message, "Source and Reference OF Pattern data different");
...
DVT_STATUS VALUE_OF_CLASS::Check (UINT32,
                                  BASE_VALUE_CLASS **,
                                  LOG_MESSAGE_CLASS *,
