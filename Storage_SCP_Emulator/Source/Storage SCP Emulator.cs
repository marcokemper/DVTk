...
namespace StorageSCPEmulator
{
	...
	public class StorageSCPEmulator : System.Windows.Forms.Form
	{
        ...
        {
            storageOptions.LoadFromFile(Path.Combine(applDirectory, "StorageSCPEmulator.ses"));
            storageCommitOptions.LoadFromFile(Path.Combine(applDirectory, "CommitSCPEmulator.ses"));
        }
		...
		private void StorageSCPEmulator_Closing(object sender, System.ComponentModel.CancelEventArgs e)
		{
			if(!isStopped)
			{
        ...
        private void menuItemCreateDicomdirForReceivedDicomMessages_Click(object sender, EventArgs e)
        {
            ...
            if (theDataDirectoryInfo.Exists)
            {
                ...
                if (dataFiles.Count != 0)
                {
                    try
                    {
                        DvtkSession.MediaSession mediaSession = DvtkSession.MediaSession.LoadFromFile(applDirectory + "\\media.ses");
