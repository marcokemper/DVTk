...
OL_VALUE_STREAM_CLASS::OL_VALUE_STREAM_CLASS()
...
bool OL_VALUE_STREAM_CLASS::SetFilename(string filename)
...
//<<===========================================================================
{
	...
	if (strstr(filename.c_str(), "B08_")) 
	{
		...
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_INFO, 1, "Using original OB 8-bit data as OL 8-bit little-endian data - may produce byte swap problems");
		}
	}
	else if (strstr(filename.c_str(), "B08C")) 
	{
		...
		if (loggerM_ptr)
		{
			loggerM_ptr->text(LOG_INFO, 1, "Using original OB 8-bit compressed data as OL 8-bit little-endian data - may produce byte swap problems");
		}
	}
	...
	else if (strstr(filename.c_str(), "W32B"))
	{
		// OW, 32 bit data, big endian
		bits_allocatedM = 32;
		fs_codeM = TS_BIG_ENDIAN;
	}
	else if (strstr(filename.c_str(), "W32L"))
	{
		// OW, 32 bit data, little endian
		bits_allocatedM = 32;
		fs_codeM = TS_LITTLE_ENDIAN;
	}
	else {
...
ATTR_VR_ENUM OL_VALUE_STREAM_CLASS::GetVR()
...
bool OL_VALUE_STREAM_CLASS::IsByteSwapRequired()
...
//<<===========================================================================
{
	...
	if (loggerM_ptr)
	{
	    if (byte_swap) 
	    {
            loggerM_ptr->text(LOG_INFO, 1, "Byte Swap of original OL data needed - details:");
        }
        else
        {
            loggerM_ptr->text(LOG_INFO, 1, "No Byte Swap of original OL data needed - details:");
        }
        switch(fs_codeM)
        {
...
bool OL_VALUE_STREAM_CLASS::StreamPatternTo(DATA_TF_CLASS& data_transfer)

//  DESCRIPTION     : Stream a OL pattern to the data_transfer.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      : 
//  NOTES           :
//<<===========================================================================
{
	...
	if (bits_allocatedM == 32) 
	{
		UINT32	value;

		// generate a simple test pattern
		const UINT nr_of_OT_in_OL = 4;
		if ((rowsM * columnsM * nr_of_OT_in_OL) != lengthM) 
		{
			if (loggerM_ptr)
			{
				loggerM_ptr->text(LOG_ERROR, 1, "OL data generation failure - rows (%d) * columns (%d) not equal length (%d)", rowsM, columnsM, lengthM);
			}
			return false;
		}
...
DVT_STATUS OL_VALUE_STREAM_CLASS::Compare(LOG_MESSAGE_CLASS *message_ptr, OB_VALUE_STREAM_CLASS &refObStream)

//  DESCRIPTION     :  Compare this against the reference OL value.
//  PRECONDITIONS   :
//  POSTCONDITIONS  :
//  EXCEPTIONS      : 
//  NOTES           :
//<<===========================================================================
{
    ...
    if (byteSwap)
    {
        ...
        sprintf (message, "Source OL File data being byte swapped before comparison with Reference OB data");
...
DVT_STATUS OL_VALUE_STREAM_CLASS::Compare(LOG_MESSAGE_CLASS *message_ptr, OL_VALUE_STREAM_CLASS &refOwStream)
...
//<<===========================================================================
{
    ...
    {
        // need to byte swap either source or reference before comparison
        sprintf (message, "Source OL File data being byte swapped before comparison with Reference OL data");
