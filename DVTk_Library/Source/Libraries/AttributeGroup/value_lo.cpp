...
bool VALUE_LO_CLASS::operator = (BASE_VALUE_CLASS &value)
...
//<<===========================================================================
{
    ...
    if (length > LO_LENGTH)
    {
        sprintf (message, "value length %i exceeds maximum length %i",
                 length, LO_LENGTH);
        messages->AddMessage (VAL_RULE_D_LO_1, message);
        result = MSG_ERROR;
    }
    ...
    if (length > 0)
    {
        ...
        DVT_STATUS result1 = EXTCHARACTERSET->IsValidExtendedChar((BYTE*)valueM.c_str(), length, ATTR_VR_LO, LO_CHAR_LENGTH, messages, specific_character_set);
...
DVT_STATUS VALUE_LO_CLASS::Compare(LOG_MESSAGE_CLASS*, BASE_VALUE_CLASS *ref_value)
...
//<<===========================================================================
{
    ...
    if (ref_value->GetVRType() != ATTR_VR_LO)
    {
...
DVT_STATUS VALUE_LO_CLASS::Compare(string value)
...
ATTR_VR_ENUM VALUE_LO_CLASS::GetVRType(void)
...
string VALUE_LO_CLASS::GetStripped(void)
