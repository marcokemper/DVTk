...
bool OD_VALUE_STREAM_CLASS::SetFilename(string filename)
...
//<<===========================================================================
{
	...
	else if (strstr(filename.c_str(), "F64B")) 
	{
		// OF, 32 bit data, big endian
		fs_codeM = TS_BIG_ENDIAN;
	}
	else if (strstr(filename.c_str(), "F64L")) 
	{
		// OF, 32 bit data, little endian
		fs_codeM = TS_LITTLE_ENDIAN;
	}
...
ATTR_VR_ENUM OD_VALUE_STREAM_CLASS::GetVR()
...
bool OD_VALUE_STREAM_CLASS::IsByteSwapRequired()
...
//<<===========================================================================
{
	...
	if (loggerM_ptr)
	{
	    if (byte_swap) 
	    {
            loggerM_ptr->text(LOG_INFO, 1, "Byte Swap of original OD data needed - details:");
        }
        else
        {
            loggerM_ptr->text(LOG_INFO, 1, "No Byte Swap of original OD data needed - details:");
        }
        switch(fs_codeM)
        {
...
void OD_VALUE_STREAM_CLASS::SwapBytes(BYTE* buffer_ptr, UINT32 length)
