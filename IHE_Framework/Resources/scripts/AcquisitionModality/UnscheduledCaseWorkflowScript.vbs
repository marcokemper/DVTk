'===============================================================================
' Filename:     UnscheduledCaseWorkflowScript.vbs
'===============================================================================
'
...
Class TestCase
    ...
    Public Sub SetUp()
		...
		' Remove some default values - for the unscheduled case
		IheFramework.AddUserDefinedDefaultTagValue(New DicomTagValueDelete(Tag.ACCESSION_NUMBER))
		IheFramework.AddUserDefinedDefaultTagValue(New DicomTagValueDelete(Tag.REQUESTED_PROCEDURE_ID))
		IheFramework.AddUserDefinedDefaultTagValue(New DicomTagValueDelete(Tag.REQUESTED_PROCEDURE_DESCRIPTION))
		IheFramework.AddUserDefinedDefaultTagValue(New DicomTagValueDelete(Tag.SCHEDULED_PROCEDURE_STEP_ID))
		IheFramework.AddUserDefinedDefaultTagValue(New DicomTagValueDelete(Tag.SCHEDULED_PROCEDURE_STEP_DESCRIPTION))
    ...
    Public Sub Execute()
		...
		If Not (acquisitionModality Is Nothing) Then
			...
			' Set the storage directory - location of images to send
			Dim storageDirectory As String = IheFramework.Config.CommonConfig.RootedBaseDirectory + "\\data\\acquisitionModality\\default"
			
            ' Create the user defined values that will simulate the values for the unscheduled case
			Dim userDefinedTags As TagValueCollection = New TagValueCollection()
			userDefinedTags.Add(New DicomTagValue(Tag.PATIENT_ID, ""))
			userDefinedTags.Add(New DicomTagValue(Tag.ACCESSION_NUMBER, ""))
			userDefinedTags.Add(New DicomTagValue(Tag.MODALITY, "CT"))
			
			' Create an unscheduled worklist item
			Dim modalityWorklistItem As DicomQueryItem
            modalityWorklistItem = acquisitionModality.CreateUnscheduledWorklistItem(userDefinedTags)				
